...
public:
  ...
  const std::vector<std::size_t>& vertex_map() const { return _vertex_map; }

  /// Map to cells of parent mesh
  const std::vector<std::size_t>& cell_map() const { return _cell_map; }

  /// Create a new Mesh based on the Meshfunction marker, where it has a value
  /// equal to tag, setting the MeshViewMapping in MeshTopology accordingly.
